...
int ServerThread(Server* server)
{
    ...
    while (1)
    {
        ...
        bytesRecv = recv(server->Connections[server->FreeID], recvbuf, 256 ,NULL);//Le thread recoit ce qu'a dit le client
        if(strcmp(recvbuf, "") == 0)
        {
            continue;
        }
        ...
        {
            server->Connections[server->FreeID] = 0;
            cout << "<Client " << server->FreeID + 1 << ":> Disconnected..." <<endl;
            return 0;
        }
        ...
        else
        {
            if (bytesRecv == SOCKET_ERROR)//Si le client c'est decconecté
            {
                ...
                {
                    WSASetLastError(0);
                    cout << "<Client " << server->FreeID + 1 << ":> User has closed the chat" <<endl;
                    string message;

                    stringstream ss;
                    ss << server->FreeID << "User has closed the chat";
        ...
        cout << "<Client " << server->FreeID + 1 << ":> " << recvbuf <<endl;//Afficher ce qu'a dit le client
        ...
        if(server->FreeID <= 9)
        {
            ss << "0" << server->FreeID << sendbuf;
        }
        else ss << server->FreeID << sendbuf;
        string temp = ss.str();//Convertion en string de la int ConCounter
